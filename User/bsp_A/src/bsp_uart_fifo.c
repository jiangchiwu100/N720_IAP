...
*********************************************************************************************************
*	函 数 名: comGetBuf
*	功能说明: 从串口缓冲区读取字符串，非阻塞。无论有无数据均立即返回
*	形    参: _ucPort: 端口号(COM1 - COM6)
*			  _pBuf: 接收到的数据存放在这个地址
*	返 回 值: 0 表示无数据, 1-65535表示读取到有效字节的长度
*********************************************************************************************************
*/
uint16_t comGetBuf(COM_PORT_E _ucPort, uint8_t *_pBuf)
{
	UART_T *pUart;
    uint16_t len = 0;
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if (USART_GetITStatus(_pUart->uart, USART_IT_RXNE) != RESET)
	{
        ...
        {             
            TIM_Cmd(TIM7, ENABLE);       //打开定时器 开始计数
            TIM_SetCounter(TIM7, 0);    //重置定时器计数值       
            
        }
        else if(_pUart->uart == USART2)
        {
           TIM_Cmd(TIM6, ENABLE);       //打开定时器 开始计数
           TIM_SetCounter(TIM6, 0);    //重置定时器计数值    
