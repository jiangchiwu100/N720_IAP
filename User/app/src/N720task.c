...
void SMS_Comm(void)
{
    ...
    {
      case SMS_Info_Set:            //设置相关信息
        {
            SMSCtrlStateMachine = 0;
            CreateGateWayConfigInfoFile();      //写入到文件系统中
            NVIC_SystemReset();
        }
        break;
      case SMS_Software_Reset:      //清表库
        {
            SMSCtrlStateMachine = 0;
            /******
            /
        ...
        break;
      case SMS_Hardware_Reset:      //MCU复位
        {
            SMSCtrlStateMachine = 0;
            NVIC_SystemReset();
        }
        break;
      case SMS_IAP_Ctrl:            //IAP操作
        {
            SMSCtrlStateMachine = 0;
            iap_load_app();     //跳转到IAP
        }
      ...
      case Enum_N720_Init_State:     //初始化N720状态
        {
			...
			else if(delay == Init_Fail)//4G有故障   1分钟之后再进行初始化 
			{
				delay = 60000;
...
//				W_State(&SysState,N720Register,true);
      ...
      case Enum_N720_JudgeAndGet:
        {            
            ...
            if(delay == Msg_Finish)     //消息接收完成 继续接收
            {
            ...
            else if(delay == Msg_Fail)  //消息接收失败 通讯有误 初始化模块      
            {
                delay = 3000;
                N720InitAllStateMachine = Enum_N720_Check_AT;
            }            
...
uint32_t N720_JudgeAndGet(void)
{
    ...
    switch(JudgeAndGetStateMachine)
    {
      ...
      case SMS_Type:            //获取SMS消息
        {
            Get_SMS_flag++;     
            JudgeAndGetStateMachine = N720_SMS_RX(Msg_Num, SMSbuf.buf, &SMSbuf.len);//获取SMS消息
            delay = 0;
        }
        break;
